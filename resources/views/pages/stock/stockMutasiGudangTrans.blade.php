            ...
            <div class="col-lg-8">
                <form action="#" id="formGlobal" class="form">
                    ...
                    <div class="form-group row">
                        <label for="jenisBayar" class="col-lg-2 col-form-label text-lg-right">Gudang Asal</label>
                        <div class="col-lg-4">
                            <select name="branchAsal" id="branchAsal" class="form-control" autocomplete="off">
                                <option disabled>Silahkan Pilih</option>
                                @php
                                    $data_branch = \App\Models\Stock\BranchStock::latest()->get();
                                    $branchAsal = $branchAsal ?? '';
                                @endphp
                                @if($data_branch->count() > 0)
                                    @foreach($data_branch as $row)
                                        <option value="{{$row->id}}" {{ ($row->id == $branchAsal) ? 'selected' : '' }}>{{$row->branchName}}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                        <label for="jenisBayar" class="col-lg-2 col-form-label text-lg-right">Gudang Tujuan</label>
                        <div class="col-lg-4">
                            <select name="branchTujuan" id="branchTujuan" class="form-control" autocomplete="off">
                                <option disabled>Silahkan Pilih</option>
                                @php
                                    $branchTujuan = $branchTujuan ?? '';
                                @endphp
                                @if($data_branch->count() > 0)
                                    @foreach($data_branch as $row)
                                        <option value="{{$row->id}}" {{ ($row->id == $branchTujuan) ? 'selected' : '' }}>{{$row->branchName}}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                    ...
                    <div class="form-group row">
                        <label class="col-lg-2 col-form-label text-lg-right">Tgl Mutasi</label>
                        <div class="col-lg-4">
                            <x-nano.input-datepicker name="tgl_mutasi" id="tgl_mutasi" value="{{ $tgl_mutasi ?? date('d-M-Y') }}" autocomplete="off"/>
                        </div>
                    </div>
                    <div class="form-group row">
    ...
    @push('scripts')
        <script>
            ...
            $('#btnSave').on('click', function(){
                $.ajax({
                    headers : {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    url : '{{ route("mutasiStock") }}',
                    method: "POST",
                    dataType : "JSON",
                    data : $('#formGlobal, #formTable').serialize(),
                    success : function (data){
                        if (data.status){
                            window.location.href = '{{ route("mutasiStock") }}';
                        }
                    },
                    error : function (jqXHR, textStatus, errorThrown){
                        $('.invalid-feedback').remove();
